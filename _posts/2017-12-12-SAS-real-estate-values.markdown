...
One of the nice things about Difference-in-Difference experiments is that they can be mathematically formulated as linear regressions - I won't bore you with [the details](https://en.wikipedia.org/wiki/Difference_in_differences#Implementation), but this means we can add in other factors to control for things that may differ between the two areas (or otherwise impact price).
